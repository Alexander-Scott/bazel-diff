...
$bazel_diff modified-filepaths $previous_revision $final_revision -w $workspace_path$workspace_subdir_path -wsd $workspace_subdir_path -b $bazel_path $modified_filepaths_output
...
git -C $workspace_path$workspace_subdir_path checkout $previous_revision --quiet

echo "Generating Hashes for Revision '$previous_revision'"
$bazel_diff generate-hashes -w $workspace_path$workspace_subdir_path -wsd $workspace_subdir_path -b $bazel_path $starting_hashes_json

git -C $workspace_path$workspace_subdir_path checkout $final_revision --quiet

echo "Generating Hashes for Revision '$final_revision'"
$bazel_diff generate-hashes -w $workspace_path$workspace_subdir_path -wsd $workspace_subdir_path -b $bazel_path -m $modified_filepaths_output $final_hashes_json

echo "Determining Impacted Targets"
$bazel_diff -sh $starting_hashes_json -fh $final_hashes_json -w $workspace_path$workspace_subdir_path -wsd $workspace_subdir_path -b $bazel_path -o $impacted_targets_path

echo "Determining Impacted Test Targets"
$bazel_diff -sh $starting_hashes_json -fh $final_hashes_json -w $workspace_path$workspace_subdir_path -wsd $workspace_subdir_path -b $bazel_path -o $impacted_test_targets_path --avoid-query "//... except tests(//...)"
